          ...
          >
            ...
            <div className="p-8 md:p-10 flex justify-between items-center border-b border-neutral-50 bg-cream-light/30">
              <div className="space-y-1">
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-accent-gold">Your Selection</span>
                <h2 className="text-2xl font-black uppercase tracking-tight">Shopping Bag</h2>
              </div>
              <button 
                onClick={() => toggleCart(false)}
            ...
            <div className="flex-grow overflow-y-auto p-8 md:p-10 custom-scrollbar">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-8 opacity-40">
                  ...
                  <p className="text-xs font-black uppercase tracking-ultra">Your bag is currently empty</p>
                  <Link 
                     href="/shop" 
                     onClick={() => toggleCart(false)}
                     className="text-[10px] font-black border-b border-primary pb-1 uppercase tracking-widest hover:text-accent-gold hover:border-accent-gold transition-colors"
                   >
                     Continue Exploring
                   </Link>
                </div>
                      ...
                      <div className="flex-grow py-2 flex flex-col justify-between">
                        <div className="space-y-1">
                           <div className="flex justify-between items-start">
                              <h3 className="text-sm font-black uppercase tracking-tight text-primary group-hover:text-accent-gold transition-colors">{item.name}</h3>
                              <button onClick={() => removeItem(item.id)} className="text-neutral-200 hover:text-red-500 transition-colors">
                                 <X size={16} />
                              </button>
                           </div>
                           <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">{item.category}</p>
                        </div>
                        <div className="flex justify-between items-end">
                          ...
                          <p className="font-black text-sm tracking-tight text-primary">฿{(item.price * item.quantity).toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="p-8 md:p-10 bg-cream-light border-t border-neutral-100 space-y-8">
                <div className="space-y-4">
                   ...
                   <div className="pt-4 border-t border-neutral-200 flex justify-between items-center">
                      <span className="text-xs font-black uppercase tracking-[0.2em]">Total Estimate</span>
                ...
                <div className="space-y-4">
                  <button className="group relative w-full bg-primary text-white h-20 rounded-full overflow-hidden transition-all active:scale-95 duration-500 shadow-luxury">
                    ...
                    <span className="relative z-10 font-black text-xs uppercase tracking-[0.3em]">Complete Acquisition</span>
                  </button>
                  <button 
                    onClick={() => toggleCart(false)}
                    className="w-full text-[10px] font-black uppercase tracking-[0.3em] text-neutral-300 hover:text-primary transition-colors text-center"
                  >
                    Continue Curation
                  </button>
                </div>
